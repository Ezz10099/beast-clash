...
- **Current implementation:** `0.2.0-representative.9` gameplay plus a query-activated Arabic fresh-cell readiness layer on `main`
...
- **Latest automated evidence:** `.9` remained GREEN: 200 active build runs, 100 choice-policy runs, 0% idle wins, 100% simple-movement wins, 8/8 deterministic replays, 8/8 selectable starting spells, isolated fresh saves, and no runtime violation. The Arabic-only batch has isolated syntax/behavior checks but not yet a complete clean-checkout run.
- **Current verification status:** Arabic source syntax and isolated localization behavior passed; repository checks now enforce English preservation, Arabic/RTL activation, essential DOM/canvas translations, whitespace stability, fresh-save compatibility, localization minification, and the existing 100 KB bundle ceiling. Full clean-checkout `npm run check` and target-phone layout remain pending because this assistant environment could not clone GitHub.
...
- Kept full launch scaling blocked and set one owner phone check as the exact next action.
