...
- `touch-controls.js` shifts touch targets upward by 56 canvas pixels so the mage stays visible above the thumb; mouse input remains exact.
- English remains the default. Arabic activation and fresh-save parameters are generated by internal tooling for tests.
- Localization and touch-control layers must not access saves or gameplay state directly.
